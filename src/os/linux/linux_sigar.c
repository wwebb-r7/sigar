...
char * sigar_dbus_get_machine_id(void) {
  return dbus_get_local_machine_id();
}

char * sigar_linux_machine_id(void) {
  ...
  /* query dbus for machine id */

  p = sigar_dbus_get_machine_id();

  if (p) {
    strncat(machine_id, ":", sizeof(char));
    strncat(machine_id, p, sizeof(machine_id) - strlen(p) - 1);
    return machine_id;
  }
